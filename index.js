...
function renderLegend() {
  ...
  selectedSections.forEach((sec, i) => {
    ...
    // console.log(sec.sessions[0]);
    let prof = [
      ...new Set(sec.sessions.map((s) => `${s?.teacher} - ${s?.type}`)),
    ];
    if (prof.includes("NN - T") || prof.includes("NN - P")) {
      prof = ["ningun profe asignado aun"];
    }
    // prof.filter((p, i) => {return prof.indexOf(p) == i;}).join(': ');
    // const prof = sec.sessions[0]?.teacher || 'Varios';
    // console.log(sec.sessions)
    ...
    item.innerHTML = `
		...
		VACANTES: ${vacantes}
                    ...
                    <div style="display: flex; align-items: start; gap: 10px; margin-top: 4px; flex-direction: column;">
                      ${prof
                        .map((p) => {
                          return `<span style="color: #666; font-size: 0.85em;">${p}</span>`;
                        })
                        .join("\n")}
                    </div>
                </div>
